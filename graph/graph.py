...
class Graph(List):
    """Grafo implementado usando lista de adyacencia"""

    class __nodeVertex:
        """Nodo que representa un vértice del grafo"""

        def __init__(self, value: Any, other_values: Optional[Any] = None):
            self.value = value  # Valor del vértice
            self.edges = List()  # Lista de aristas salientes
            self.edges.add_criterion('value', Graph._order_by_value)
            self.edges.add_criterion('weight', Graph._order_by_weight)
            self.other_values = other_values
            self.visited = False  # Para algoritmos de recorrido
        
        def __str__(self):
            return self.value
    
    class __nodeEdge:
        """Nodo que representa una arista del grafo"""
    ...
    def __init__(self, is_directed=False):
        """Inicializa el grafo. is_directed=True para grafo dirigido"""
        self.add_criterion('value', self._order_by_value)
        self.is_directed = is_directed  # False = no dirigido, True = dirigido

    def _order_by_value(item):
        """Criterio de ordenamiento por valor del vértice"""
        return item.value

    def _order_by_weight(item):
        """Criterio de ordenamiento por peso de la arista"""
        return item.weight
    ...
    def exist_path(self, origin, destination):
        """Verifica si existe un camino entre origen y destino usando DFS"""
            ...
            if vertex_pos is not None:
                if not graph[vertex_pos].visited:
                    ...
                    if graph[vertex_pos].value == destination:
                        ...
                        # Buscar recursivamente en los vecinos
    ...
    def deep_sweep(self, value) -> None:
        ...
        def __deep_sweep(graph, value):
            ...
            if vertex_pos is not None:
                if not graph[vertex_pos].visited:
                    graph[vertex_pos].visited = True
                    print(graph[vertex_pos])  # Procesar el vértice
                    # Visitar recursivamente todos los vecinos
                    for edge in graph[vertex_pos].edges:
                        destination_edge_pos = graph.search(edge.value, 'value')
                        if not graph[destination_edge_pos].visited:
    ...
    def amplitude_sweep(self, value)-> None:
        """Recorrido en amplitud (BFS) del grafo usando cola"""
        queue_vertex = Queue()
        self.mark_as_unvisited()
        ...
        if vertex_pos is not None:
            if not self[vertex_pos].visited:
                ...
                queue_vertex.arrive(self[vertex_pos])  # Agregar a la cola
                while queue_vertex.size() > 0:
                    vertex = queue_vertex.attention()  # Sacar de la cola
                    print(vertex.value)  # Procesar el vértice
    ...
    def dijkstra(self, origin):
        ...
        no_visited = HeapMin()  # Cola de prioridad para vértices no visitados
        path = Stack()  # Pila para almacenar el camino resultado
        # Inicializar distancias: 0 para origen, infinito para el resto
        for vertex in self:
            distance = 0 if vertex.value == origin else inf
            no_visited.arrive([vertex.value, vertex, None], distance)
        # Procesar vértices en orden de distancia mínima
        while no_visited.size() > 0:
            value = no_visited.attention()  # Vértice con menor distancia
            costo_nodo_actual = value[0]
            path.push([value[1][0], costo_nodo_actual, value[1][2]])
            edges = value[1][1].edges
            # Relajar aristas: actualizar distancias si se encuentra camino más corto
            for edge in edges:
                pos = no_visited.search(edge.value)
                if pos is not None:
                    if pos is not None:
                        if costo_nodo_actual + edge.weight < no_visited.elements[pos][0]:
                            no_visited.elements[pos][1][2] = value[1][0]  # Actualizar predecesor
    ...
    def kruskal(self, origin_vertex):
        ...
        def search_in_forest(forest, value):
            """Busca en qué componente del bosque está un vértice"""
            for index, tree in enumerate(forest):
                if value in tree:
                    return index
                
        forest = []  # Bosque de componentes conexas
        edges = HeapMin()  # Cola de prioridad con todas las aristas
        ...
        while len(forest) > 1 and edges.size() > 0:
            edge = edges.attention()  # Arista de menor peso
            origin = search_in_forest(forest, edge[1][0])
            destination = search_in_forest(forest, edge[1][1])
            if origin is not None and destination is not None:
                # Si los vértices están en componentes diferentes, unirlas
                if origin != destination:
                    # Eliminar las dos componentes y crear una nueva unida
                    if origin > destination:
                    ...
                    # Formato del string depende de si son vértices simples o árboles
